   ...
   [
    ...
    (0, 0, 10,
     [(multiplayer_is_server),
      ...
      (gt, ":plants_count", 0),
      (try_begin),
        (lt, "$g_loop_plant_to_check", ":plants_count"),
        (store_add, ":plant_slot", "$g_loop_plant_to_check", slot_removed_plants_begin),
        (troop_get_slot, ":instance_id", "trp_removed_scene_props", ":plant_slot"),
        (try_begin),
          (le, ":instance_id", 0),
        (else_try),
          (neg|scene_prop_slot_eq, ":instance_id", slot_scene_prop_state, scene_prop_state_hidden),
          (troop_set_slot, "trp_removed_scene_props", ":plant_slot", -1),
        (else_try),
          (scene_prop_get_slot, ":regen_time", ":instance_id", slot_scene_prop_state_time),
          (store_mission_timer_a, ":time"),
          (ge, ":time", ":regen_time"),
          (call_script, "script_regrow_plant", ":instance_id"),
          (troop_set_slot, "trp_removed_scene_props", ":plant_slot", -1),
        (try_end),
        (val_add, "$g_loop_plant_to_check", 1),
      (try_end),
      (ge, "$g_loop_plant_to_check", ":plants_count"),
