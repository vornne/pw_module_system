...
mission_templates = [
  ("conquest", mtf_battle_mode, -1, "Fight for control of the castles.",
   ...
   [
    ...
    (redraw_castle_banners_interval, 0, 0, [],
     [(multiplayer_is_server),
    ...
    (0, 0, 0, [],
     [(key_clicked, key_f6),
      (try_begin),
        (eq, "$g_snow_falling", 1),
        (assign, "$g_snow_falling", 0),
      (else_try),
        (assign, "$g_snow_falling", 1),
        (assign, "$g_fog_distance", 1000),
      (try_end),
      ]),
    ...
    (0, 0, 0.1, [],
     [(neg|multiplayer_is_server),
      (try_begin),
        (eq, "$g_snow_falling", 1),
        (store_random_in_range, ":mul_fog", 90, 104),
        (val_mul, "$g_fog_distance", ":mul_fog"),
        (val_div, "$g_fog_distance", 100),
        (val_max, "$g_fog_distance", 100),
        (mission_cam_get_position, pos1),
        (init_position, pos2),
        (position_copy_rotation, pos1, pos2),
        (multiplayer_get_my_player, ":player_id"),
        (player_get_agent_id, ":agent_id", ":player_id"),
        (agent_get_speed, pos3, ":agent_id"),
        (position_get_y, ":forwards_speed", pos3),
        (assign, ":snow_height", 1000),
        (val_div, ":forwards_speed", 2),
        (val_sub, ":snow_height", ":forwards_speed"),
        (val_max, ":snow_height", 100),
        (position_move_z, pos1, ":snow_height", 1),
        (particle_system_burst, "psys_snow", pos1, 100),
      (else_try),
        (val_mul, "$g_fog_distance", 105),
        (val_div, "$g_fog_distance", 100),
      (try_end),
      (try_begin),
        (is_between, "$g_fog_distance", 100, 10000),
        (set_fog_distance, "$g_fog_distance", 0xAAAAAA),
      (try_end),
      ]),
