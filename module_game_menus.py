...
#				(eq, "$g_player_minister", ":companion"),
			...
			(else_try),	
				...
				(try_begin),
					...
					(display_message, "@{!}DEBUG: current mission: {reg2}, days on mission: {reg3}, prisoner: {reg4}"),
				(try_end),
			(try_end),	
 ...
 #                                   (assign, "$cant_leave_encounter", 0),
  ...
  (
    ...
    [
     ...
     (else_try),
	   (ge, "$cheat_mode", 1),
       (display_message, "str_debug__you_fought_with_a_center_so_no_change_in_morale"),
     (try_end),
        ...
        (else_try),
          ...
          (try_for_range, ":stack_no", "$last_defeated_hero", ":num_stacks"), #May 31 bug note -- this now returns some heroes in victorious party as well as in the other party
			...
			(else_try),
			  ...
			  #New for Warband 0.631 to fix bug, hopefully...
          ...
          (else_try),
            ...
            (try_begin), #my kingdom
              ...
              (else_try), #player took a walled center while he is a vassal of rebels.
                ...
                (change_screen_return),              
                (start_map_conversation, ":faction_leader"),
              (else_try), #player took a walled center for player's kingdom
                ...
                (try_for_range, ":stack_no", 0, ":num_stacks"),
                  ...
                  #(assign, reg2, ":num_routed"),
                  #(display_message, "@{!}DEBUG : all : {reg0}, wounded : {reg1}, routed : {reg2}"),
                  ...
                  (try_begin),
                    (eq, "$cheat_mode", 1),
                    (assign, reg6, ":troop_score"),
                    (str_store_troop_name, s6, ":stack_troop"),
                    (display_message, "@{!}DEBUG : troop name : {s6} ; troop score : {reg6}"),
                  (try_end),
                ...
                (start_map_conversation, ":best_troop"),          
              (try_end),
            (try_end),
            #(else_try), #rebellion
            #  (eq, "$g_next_menu", "mnu_castle_taken"), 
...
##        (display_message, "@{!}enemy_party={reg3} ally_party={reg4}"),
  ...
  (
    ...
    [
		...
		(store_faction_of_party, ":fortress_faction", "$g_encountered_party"),
		(try_begin),
			(assign, ":damage", 20),
			(is_between, "$g_encountered_party", towns_begin, towns_end),
			(assign, ":damage", 40),
		(try_end),
		(call_script, "script_faction_inflict_war_damage_on_faction", ":faction_no", ":fortress_faction", ":damage"),
...
{reg1? You may station troops here to defend it against enemies who may try to recapture it. Also, you should select now whether you will hold the {reg2?town:castle} yourself or give it to a faithful vassal...:}",# Only visible when castle is taken without being a vassal of a kingdom.
    ...
    [
        ...
        (try_begin),
		  (eq, 1, 0), #this should not occur here -- in this case, the old, enemy lord thinks that the player has given him the castle
          (party_get_slot, ":center_object_lord", "$g_encountered_party", slot_town_lord),
          (neq, ":center_object_lord", "trp_player"),          
          (call_script, "script_add_log_entry", logent_castle_given_to_lord_by_player, "trp_player", "$g_encountered_party", -1, "$g_encountered_party_faction"),
		  #Also, "-1" should be troop object
          #add log entry to "lord x" about "thank player because he gave this castle to lord x".
        (try_end),
        
        #removed from here because here already owner of castle become changed.
		...
		(try_begin),
			...
			(assign, ":damage", 40),
		(try_end),
		(call_script, "script_faction_inflict_war_damage_on_faction", "$players_kingdom", ":fortress_faction", ":damage"),
      ...
      ("town_dungeon",
       ...
       [	   
           (try_begin),
			...
			(faction_slot_eq, "$g_encountered_party_faction", slot_faction_prison_guard_troop, ":agent_type"),
...
#      (else_try),
    ...
    [
      ...
      ("continue",[],"Tell the woman to inform her mistress that you will come shortly",
       [
	    ...
	    (str_store_troop_name, s3, ":lady_to_visit"),
	    (str_store_party_name, s4, "$g_notification_menu_var2"),
  ...
  ( #pre lady visit
	...
	(else_try), #the circumstances under which the lady arranges for a surreptitious entry
		...
		(try_begin),
			...
			(this_or_next|ge, ":hours_since_last_visit", 168), #at least a week
 ...
 This will surely be your best opportunity to attack him.",
        ...
        (try_begin),
          ...
          (try_for_range, ":stack_no", 0, ":num_stacks"),
            ...
            (try_begin),              
              ...
              (str_store_troop_name, s7, ":stack_troop"),
              (display_message, "@{reg2} {s7} died."),
              (gt, ":stack_size", 0),              
              (party_add_members, "p_total_enemy_casualties", ":stack_troop", ":stack_size"), #addition_to_p_total_enemy_casualties
              (party_stack_get_num_wounded, ":stack_wounded_size", "p_temp_casualties", ":stack_no"),
              (gt, ":stack_wounded_size", 0),
  ...
  ("notification_relieved_as_marshal", mnf_disable_all_keys,
    "{s4} wishes to inform you that your services as marshal are no longer required. In honor of valiant efforts on behalf of the realm over the last {reg4} days, however, {reg5?she:he} offers you a purse of {reg5} denars.",
    "none",
    [
