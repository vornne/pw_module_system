  ...
  ("escape_menu", prsntf_manual_end_only, 0,
   [(ti_on_presentation_load,
     [(set_fixed_point_multiplier, 1000),
      ...
      (assign, "$g_quit_waiting_time_limit", 0),
    ...
    (ti_on_presentation_event_state_change,
     [(store_trigger_param_1, ":object"),
      ...
      (else_try),
        ...
        (try_begin), # request the server to quit cleanly
          (eq, "$g_quit_waiting_time_limit", 0),
          (multiplayer_send_message_to_server, client_event_request_quit),
          (store_add, "$g_quit_waiting_time_limit", ":time", quit_waiting_interval),
        (else_try), # if the server doesn't reply within the time limit, allow forced disconnection
          (gt, ":time", "$g_quit_waiting_time_limit"),
          (finish_mission, 0),
        (try_end),
      (try_end),
      ]),
    (ti_on_presentation_run,
     [(store_trigger_param_1, ":cur_time"),
      ...
      (try_end),
      (try_begin),
        (gt, "$g_quit_waiting_time_limit", 0),
        (store_mission_timer_a, ":time"),
        (gt, ":time", "$g_quit_waiting_time_limit"),
        (overlay_set_color, "$g_presentation_obj_escape_menu_quit", 0xFFFF0000),
        (overlay_set_text, "$g_presentation_obj_escape_menu_quit", "str_force_quit"),
      (try_end),
      ]),
    ]),
