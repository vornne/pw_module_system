...
To deposit gold in a chest you must be near it, and to withdraw you also either need to be the lord of the associated faction, break the chest open with a weapon, or use a lock pick to open it (looting skill makes success more likely). To tell if a chest is unlocked, press and release the use button: if you hear a click sound it is locked, but if no sound it is unlocked; locked teleport doors work the same way, except that they are relocked by using the door as a member of the faction owning the castle. \
